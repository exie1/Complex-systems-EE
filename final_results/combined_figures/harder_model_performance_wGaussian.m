...
tic
[X1,t] = fHMC_optDynamic(p,avg,payoffs,1.5);
[X2,~] = fHMC_optDynamic(p,avg,payoffs,2);

[reward1,~] = payoffDynamicEnd(X1,p,payoffs);
[reward2,~] = payoffDynamicEnd(X2,p,payoffs);
%%
[~,spatial_FNS] = payoffDynamicEnd(X1(:,:,2),p,payoffs);
%%
toc
...
tic
parfor i = 1:avg
    [history1, ~] = dUCB(p,payoffs,0.98,0.4);
    history_dUCB(i,:) = history1(2,:);
end
toc

histories = {reward1, reward2, history_dUCB};
labels = {'FNS','FNS w/ Gaussian noise','dUCB'};
colours = {[0 0.4470 0.7410],[0.8500 0.3250 0.0980],...
        [0.9290 0.6940 0.1250]};
thick = {1.5,1,1};

clear reward1 reward2 history_dUCB
