...
toc
%%
% plotSwitching(X,t,payoffs,history_FNS,spatial_FNS,depth_FNS);
% plotSwitching(X2,t,payoffs,history_FNS2,spatial_FNS2,depth_FNS2);

plotSwitching(X,t,payoffs,spatial_FNS)
% plotSwitching(X2,t,payoffs,spatial_FNS2)
...
[history_dUCB, spatial_dUCB] = dUCB(p, payoffs, 0.96, 2);   
...
% plotChoices(payoffs,history_FNS,history_dUCB,history_FNS2,history_softmax);
...
history_FNS2 = zeros(averages,payoff_time);
...
parfor average = 1:averages
    ...
    [~,~,history4,~,~] = fHMC_MAB_sw(p,payoffs,2,1.5,0.5);
    ...
    history_FNS2(average,:) = history4(2,:);
...
%%
plotCumulativeAvg(history_FNS,history_FNS2,history_dUCB,history_softmax)
...
%     plot(t,X(1,:),'LineWidth',1)
    plot(linspace(t(1),t(end),payoff_time), spatial_FNS(1,:),'LineWidth',1)
    plot(linspace(t(1),t(end),payoff_time),pi/2*(payoffs(:,2)-3)/2,'LineWidth',2)
...
% function plotSwitching(X,t,payoffs,history_FNS,spatial_FNS,depth_FNS)
%     payoff_time = size(payoffs,1); 
% 
%     figure
%     subplot(3,1,1)
...
%     legend('Walker points','Sampled points','Selected Option')
% 
%     subplot(3,1,3)
%     plot(depth_FNS')
% end
...
function plotCumulativeAvg(reward1,reward2,reward3,reward4)
    ...
    plot(x,y2,'-.','DisplayName','FNS w/ Gaussian performance','LineWidth',1)
    plot(x,y3,':','DisplayName','dUCB performance','LineWidth',1)
    ...
    fill([x;flipud(x)],[y2-dy2 ; flipud(y2+dy2)],[0.8500 0.3250 0.0980], ...
        'linestyle', 'none','FaceAlpha',0.15,'HandleVisibility','off')
    ...
    fill([x;flipud(x)],[y4-dy4 ; flipud(y4+dy4)],[0.4940 0.1840 0.5560], ...
        'linestyle', 'none','FaceAlpha',0.05,'HandleVisibility','off')
