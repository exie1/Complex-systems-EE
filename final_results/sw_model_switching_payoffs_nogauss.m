...
clear p
payoffs = (csvread('payoffs_randstep.csv')')*0.8+0.2;
...
% [X2,~,history_FNS2,spatial_FNS2,depth_FNS2] = fHMC_MAB_sw(p,payoffs,2,1.5,0);
toc
%%
X2 = X(:,1);
for i = 201:length(t)
...
plotSwitching(X2,t,payoffs,spatial_FNS)
ylim([-4,4])
...
parfor average = 1:averages
    [~,~,history0,~,~] = fHMC_MAB_sw(p,payoffs,1.5,2,1);
    [history1, ~] = dUCB(p,payoffs,0.96,0.3);
    [history2, ~] = UCB(p,payoffs,0.5);
...
%     plot(t,X(1,:),'LineWidth',1)
    plot(linspace(t(1),1000,size(X,2)), X(1,:),'LineWidth',1)
    plot(linspace(t(1),1000,1000),pi/2*(payoffs(:,2)-0.6)/0.4,'LineWidth',2)
