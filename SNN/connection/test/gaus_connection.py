...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep 28 12:53:42 2020
...
class get_ijwd:
    ...
    def generate_ijw(self):
        ...
        self.i_ee, self.j_ee, _ = cn.connect_2lattice.gaussian_decay(self.e_lattice, self.e_lattice, np.arange(self.Ne), self.width, periodic_boundary=True, interarea_dist=0, \
                                                                     peak_p=self.p_peak_ee, sig_d=self.decay_p_ee, src_equal_trg = True, self_cnt = False)
        
        self.i_ei, self.j_ei, _ = cn.connect_2lattice.gaussian_decay(self.e_lattice, self.i_lattice, np.arange(self.Ne), self.width, periodic_boundary=True, interarea_dist=0, \
                                                                     peak_p=self.p_peak_ei, sig_d=self.decay_p_ei, src_equal_trg = False, self_cnt = False)
        
        self.i_ie, self.j_ie, _ = cn.connect_2lattice.gaussian_decay(self.i_lattice, self.e_lattice, np.arange(self.Ni), self.width, periodic_boundary=True, interarea_dist=0, \
                                                                     peak_p=self.p_peak_ie, sig_d=self.decay_p_ie, src_equal_trg = False, self_cnt = False)
        
        self.i_ii, self.j_ii, _ = cn.connect_2lattice.gaussian_decay(self.i_lattice, self.i_lattice, np.arange(self.Ni), self.width, periodic_boundary=True, interarea_dist=0, \
                                                                     peak_p=self.p_peak_ii, sig_d=self.decay_p_ii, src_equal_trg = True, self_cnt = False)
    ...
    def find_peak_probability(self, mean_SynNum, tau_d, lattice, width, position=[0,0]):
        ...
        p_peak = mean_SynNum/(np.sum(np.exp(-0.5*(dist/tau_d)**2)))
        if p_peak > 1:
