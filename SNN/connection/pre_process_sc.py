...
class get_ijwd:
    '''
        get intra-area connectiviy (pre/post-synaptic neurons, weight, and delay)
    '''
    def __init__(self, *param_dict, **kwargs):
        
        self.Ne = 4096 # number of exitatory neurons
        self.Ni = 1024 # number of inhibitory neurons
        self.width = 64 # width of network (in number of neurons)
        ...
        ''' mean in-degree of neurons'''
        self.mean_SynNumIn_ee = 320  # mean in-degree of e-e connection    ; ### SynNumOut: same # p = 0.08
        self.mean_SynNumIn_ei = 500  # mean in-degree of e-i connection     ; ### SynNumOut: /4 # p = 0.125
        self.mean_SynNumIn_ie = 200  # mean in-degree of i-e connection     ; ### SynNumOut: *4 # p = 0.2
        self.mean_SynNumIn_ii = 250  # mean in-degree of i-i connection     ; ### SynNumOut: same # p = 0.25
        
        '''mean synaptic weights; in nsiemens'''
        self.w_ee_mean = 4 # mean e-e weight; nsiemens
        self.w_ei_mean = 5 # mean e-i weight; nsiemens
        self.w_ie_mean = 20 # mean  i-e weight; nsiemens
        self.w_ii_mean = 25 # mean i-i weight; nsiemens
        
        self.delay = [0.5,2.5] # # lower and upper bound of synaptic delay, millisecond
    ...
    def generate_ijw(self):
        ...
        '''find peak connection probability based on in-degree and decay constant'''
        '''connection probability is a exponential function of distance, which peaks at distance=0'''
        self.p_peak_ee = self.find_peak_probability(self.mean_SynNumIn_ee, self.decay_p_ee, self.e_lattice, self.width, position=self.e_lattice[0], cnt_type='E to E')
        self.p_peak_ei = self.find_peak_probability(self.mean_SynNumIn_ei, self.decay_p_ei, self.e_lattice, self.width, position=self.i_lattice[0], cnt_type='E to I')
        self.p_peak_ie = self.find_peak_probability(self.mean_SynNumIn_ie, self.decay_p_ie, self.i_lattice, self.width, position=self.e_lattice[0], cnt_type='I to E')
        self.p_peak_ii = self.find_peak_probability(self.mean_SynNumIn_ii, self.decay_p_ii, self.i_lattice, self.width, position=self.i_lattice[0], cnt_type='I to I')

        ''' i: index of pre-synaptic neurons
            j: index of post-synaptic neurons
            dist: distance between neurons'''
        self.i_ee, self.j_ee, self.dist_ee = cn.connect_2lattice.expo_decay(self.e_lattice, self.e_lattice, np.arange(self.Ne), self.width, periodic_boundary=True, interarea_dist=0, \
                                       peak_p=self.p_peak_ee, tau_d=self.decay_p_ee, src_equal_trg = True, self_cnt = False)
        
        self.i_ei, self.j_ei, self.dist_ei = cn.connect_2lattice.expo_decay(self.e_lattice, self.i_lattice, np.arange(self.Ne), self.width, periodic_boundary=True, interarea_dist=0, \
                                       peak_p=self.p_peak_ei, tau_d=self.decay_p_ei, src_equal_trg = False, self_cnt = False)
        
        self.i_ie, self.j_ie, self.dist_ie = cn.connect_2lattice.expo_decay(self.i_lattice, self.e_lattice, np.arange(self.Ni), self.width, periodic_boundary=True, interarea_dist=0, \
                                       peak_p=self.p_peak_ie, tau_d=self.decay_p_ie, src_equal_trg = False, self_cnt = False)
        
        self.i_ii, self.j_ii, self.dist_ii = cn.connect_2lattice.expo_decay(self.i_lattice, self.i_lattice, np.arange(self.Ni), self.width, periodic_boundary=True, interarea_dist=0, \
                                       peak_p=self.p_peak_ii, tau_d=self.decay_p_ii, src_equal_trg = True, self_cnt = False)
        
        '''number of in-degree for each neuron'''
        ...
        '''
        w: synaptic weights
        mean incomming synaptic weight for each neuron is proportional to J/sqrt(in-degree)'''
    ...
    def generate_d_rand(self):
        '''synaptic delay uniformly distributed between 0 to self.delay
            self.delay is a single number'''
    ...
    def generate_d_dist(self):
        '''synaptic delay proportional to distance between neurons
            self.delay is an 1*2 array'''
        self.d_ee = self.dist_ee/(2**0.5*self.width/2)*(self.delay[1] - self.delay[0]) + self.delay[0]
        self.d_ie = self.dist_ie/(2**0.5*self.width/2)*(self.delay[1] - self.delay[0]) + self.delay[0]
        self.d_ei = self.dist_ei/(2**0.5*self.width/2)*(self.delay[1] - self.delay[0]) + self.delay[0]
        self.d_ii = self.dist_ii/(2**0.5*self.width/2)*(self.delay[1] - self.delay[0]) + self.delay[0]
    
    def generate_d_const(self):
        '''constant synaptic delay'''
        self.d_ee = self.delay
        self.d_ie = self.delay
        self.d_ei = self.delay
        self.d_ii = self.delay

    def find_peak_probability(self, mean_SynNum, tau_d, lattice, width, position=[0,0], cnt_type='neurons'):
    
        dist = cn.coordination.lattice_dist(lattice, width, position)
        p_peak = mean_SynNum/(np.sum(np.exp(-dist/tau_d)))
        if p_peak > 1:
            warnings.warn(" Error: peak connection probability of %s exceeds '1', adjust 'decay_p' or 'mean_SynSum' to reduce it below '1'"%cnt_type )
